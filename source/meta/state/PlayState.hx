...
class PlayState extends MusicBeatState
{
	...
	public static var vocals2:FlxSound;
	...
	public static var madgirl:FlxSprite;
	...
	public static var healthgain:Bool = true;
	public static var deathanim:Bool = true;
	...
	public static var bumpSwitch:Bool = false;
	public static var bSwitch:Bool = false;

	public static var lighting1:FlxSprite;
	public static var lighting2:FlxSprite;
	...
	public static var animationOFF:Bool;
	...
	override public function create()
	{
		...
		{
			case 'lovestruck':
				dadOpponent = new Character().setCharacter(50, 850, '2dad');
				add(dadOpponent);
				remove(dadOpponent);
			case 'heartbeat':
				dadOpponent = new Character().setCharacter(50, 850, '2dad');
				add(dadOpponent);
				remove(dadOpponent);
				dadOpponent = new Character().setCharacter(50, 850, 'simpgirlglow');
				add(dadOpponent);
				remove(dadOpponent);
				gf.setCharacter(300, 100, 'biddlegfglow');
				add(gf);
				remove(gf);
		}
		...
		camPos = new FlxPoint(gf.getMidpoint().x - 100, boyfriend.getMidpoint().y - 100);
		
		lighting1 = new FlxSprite(0, -500).loadGraphic(Paths.image('backgrounds/lighting/Blue'));
		FlxTween.tween(lighting1, {y: -50}, 4, {type: FlxTweenType.PINGPONG, ease: FlxEase.sineInOut, loopDelay: 0});
		lighting1.scale.set(5, 2);
		lighting1.visible = false;

		lighting2 = new FlxSprite(0, -500).loadGraphic(Paths.image('backgrounds/lighting/Pink'));
		FlxTween.tween(lighting2, {y: -50}, 1.1, {type: FlxTweenType.PINGPONG, ease: FlxEase.sineInOut, loopDelay: 0});
		lighting2.scale.set(5, 2);
		lighting2.visible = false;

		switch (SONG.song.toLowerCase())
		{
			case 'heartbeat':
				death = new FlxSprite(820, 432);
				death.frames = Paths.getSparrowAtlas('death/BIDDLEDEADFRIEND');
			case 'infatuated':
				death = new FlxSprite(700, 400);
				death.frames = Paths.getSparrowAtlas('death/SALTYDEAD');
			default:
				death = new FlxSprite(820, 432);
				death.frames = Paths.getSparrowAtlas('death/DEADFRIEND');
		}
		death.animation.addByPrefix('die', "BF dies", 24, false);
		death.antialiasing = true;
		death.visible = false;
		death.updateHitbox();
		...
		{
		case 'bliss':
			var evilTrail = new FlxTrail(dadOpponent, null, 4, 24, 0.3, 0.069);
			add(evilTrail);
		case 'heartbeat':
				boyfriend.setCharacter(750, 850, 'biddleglow');
				add(boyfriend);
				remove(boyfriend);
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.SPACE)
		{
			...
			FlxG.sound.play(Paths.sound('EYY'));
		}	
		...
		if (!inCutscene) {
			...
			if (health <= 0 && startedCountdown)
			{
				...
				if (deathanim == true)
				{
					FlxG.sound.play(Paths.sound('boyfriend-dead'));
					FlxG.camera.shake(0.2, 0.2);
					death.animation.play('die', true);
				}
				healthgain = false;
				health = 0;
				deathanim = false;
	...
	function noteCalls()
	{
			...
			&& (!holdControls.contains(true) || boyfriendStrums.autoplay)))
		{
			if (boyfriend.animation.curAnim.name.startsWith('sing')
				...
				&& !boyfriend.animation.curAnim.name.endsWith('hey'))
				boyfriend.dance();
		}
	}
	...
	function missNoteCheck(?includeAnimation:Bool = false, direction:Int = 0, character:Character, popMiss:Bool = false, lockMiss:Bool = false)
	{
		if (includeAnimation)
		{
			...
			if (!character.animation.curAnim.name.startsWith('hey'))
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
		{
			...
			vocals2 = new FlxSound().loadEmbedded(Paths.voices2(SONG.song), false, true);
		}	
		else
			vocals = new FlxSound();
	...
	override function stepHit()
	{
		...
		if (songMusic.time >= Conductor.songPosition + 20 || songMusic.time <= Conductor.songPosition - 20)
			resyncVocals();
		//*/
		if (curSong.toLowerCase() == 'lovestruck')
		{
			switch(curStep)
			{
				case 1055:
					remove(dadOpponent);
					dadOpponent.setCharacter(50, 850, '2dad');
					add(dadOpponent);	
			}
		...
		if (curSong.toLowerCase() == 'heartbeat')
		{
			switch (curStep)
			{
				case 470, 1110:
				...
				case 639, 1151:
					remove(dadOpponent);
					dadOpponent.setCharacter(50, 850, 'simpgirlglow');
					add(dadOpponent);
					remove(boyfriend);
					boyfriend.setCharacter(850, 850, 'biddleglow');
					add(boyfriend);
					remove(gf);
					gf.setCharacter(300, 100, 'biddlegfglow');
					add(gf);
				case 895:
					remove(boyfriend);
					boyfriend.setCharacter(850, 850, 'biddlebf');
					add(boyfriend);
					remove(dadOpponent);
					dadOpponent.setCharacter(50, 850, '2dad');
					add(dadOpponent);
					remove(gf);
					gf.setCharacter(300, 100, 'biddlegf');
					add(gf);
			}
			switch(curStep)
			{
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'fondness')
		{
			switch (curBeat)
			{
				case 28, 224, 288:
					defaultCamZoom = 1.2;
					dadOpponent.characterData.camOffsetY = 50;
					FlxTween.tween(ClassHUD.bar1, {y: -120}, 0.2, {type: FlxTweenType.ONESHOT, ease: FlxEase.sineOut});
					FlxTween.tween(ClassHUD.bar2, {y: 800}, 0.2, {type: FlxTweenType.ONESHOT, ease: FlxEase.sineOut});
				case 88, 256, 296:
					defaultCamZoom = 0.9;
					dadOpponent.characterData.camOffsetY = 100;
					dadOpponent.characterData.camOffsetX = 0;
					FlxTween.tween(ClassHUD.bar1, {y: -800}, 1, {type: FlxTweenType.ONESHOT, ease: FlxEase.sineOut});
					FlxTween.tween(ClassHUD.bar2, {y: 1500}, 1, {type: FlxTweenType.ONESHOT, ease: FlxEase.sineOut});
			}
		...
		if (curSong.toLowerCase() == 'heartbeat')
		{
			...
			{
				case 159, 223, 287:
					FlxTween.tween(boyfriend, {alpha: 0}, 0.2, {type: ONESHOT});
					FlxTween.tween(dadOpponent, {alpha: 0}, 0.2, {type: ONESHOT});
					FlxTween.tween(gf, {alpha: 0}, 0.2, {type: ONESHOT});
				case 160, 224, 288:
					FlxTween.tween(boyfriend, {alpha: 1}, 1, {type: ONESHOT});
					FlxTween.tween(dadOpponent, {alpha: 1}, 1, {type: ONESHOT});
					FlxTween.tween(gf, {alpha: 1}, 1, {type: ONESHOT});
				case 352:
					FlxTween.tween(boyfriend, {alpha: 0}, 4, {type: ONESHOT});
				case 360:
					FlxTween.tween(dadOpponent, {alpha: 0}, 4, {type: ONESHOT});
				case 368:
					gf.animation.play('cheer');
					FlxTween.tween(gf, {alpha: 0}, 4, {type: ONESHOT});
			}
			switch(curBeat)
			{
				case 32, 144:
				...
				case 368:
					defaultCamZoom = 0.8;
					dadOpponent.characterData.camOffsetX = 290;
					dadOpponent.characterData.camOffsetY = -50;
			}
			switch (curBeat)
			{
		...
		if (curSong.toLowerCase() == 'infatuated')
		{
			switch(curBeat)
			{
				case 80, 112, 144, 180, 188, 208, 312, 348, 356:
					bumpSwitch = true;
				case 100, 128, 176, 184, 192, 224, 344, 352, 360:
					bumpSwitch = false;
			}

			switch(curBeat)
			{
				case 44, 112, 140, 176, 184, 192, 272, 344, 352:
					defaultCamZoom = 1.2;
					dadOpponent.characterData.camOffsetY = 50;
					FlxTween.tween(ClassHUD.bar1, {y: -120}, 0.2, {type: FlxTweenType.ONESHOT, ease: FlxEase.sineOut});
					FlxTween.tween(ClassHUD.bar2, {y: 800}, 0.2, {type: FlxTweenType.ONESHOT, ease: FlxEase.sineOut});
				case 80, 128, 144, 180, 188, 204, 304, 348, 356:
					defaultCamZoom = 0.9;
					dadOpponent.characterData.camOffsetY = 100;
					dadOpponent.characterData.camOffsetX = 0;
					FlxTween.tween(ClassHUD.bar1, {y: -800}, 1, {type: FlxTweenType.ONESHOT, ease: FlxEase.sineOut});
			...
			{
				case 48:
					lighting1.visible = true;
					lighting2.visible = true;
					Stage.ftween.start();
			}
		}
		...
		if (curSong.toLowerCase() == 'lovestruck')
		{
			...
			switch(curBeat)
			{
				...
				case 260:
					defaultCamZoom = 0.8;
					dadOpponent.characterData.camOffsetX = 290;
					dadOpponent.characterData.camOffsetY = 150;
				case 263:
					gf.animation.play('cheer');
					bSwitch = true;
				case 264:
	...
	private function callDefaultSongEnd()
	{
		...
		ForeverTools.killMusic([songMusic, vocals, vocals2]);
	...
	function callTextbox() {
		...
		if (openfl.utils.Assets(dialogPath))
		{
