...
class OptionsMenuState extends MusicBeatState
{
	...
	private var checker1:FlxSprite;
	private var checker2:FlxSprite;
	private var shading:FlxSprite;

	public static var overlayTOP:FlxSprite;
	public static var overlayBOTTOM:FlxSprite;
	...
	private function returnSubgroup(groupName:String):FlxTypedGroup<Alphabet>
	{
		...
		for (i in 0...categoryMap.get(groupName)[0].length)
		{
			...
			{
				var thisOption:Alphabet = new Alphabet(0, 370, categoryMap.get(groupName)[0][i][0], true, false);
	...
	public function openControlmenu()
	{
		if (controls.ACCEPT)
		{
			FlxG.sound.play(Paths.sound('confirmMenu'));
			lockedMovement = true;
			FlxTween.tween(overlayBOTTOM, {y: 1000}, 1.5, {type: FlxTweenType.ONESHOT, ease: FlxEase.expoIn, loopDelay: 0});
			FlxTween.tween(overlayTOP, {y: -500}, 1.5, {type: FlxTweenType.ONESHOT, ease: FlxEase.expoIn, loopDelay: 0});
			FlxFlicker.flicker(activeSubgroup.members[curSelection], 1.5, 0.06 * 2, true, false, function(flick:FlxFlicker)
			{
			  #if android
			  removeVirtualPad();
			  #end
