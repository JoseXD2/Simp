...
class OptionsSubstate extends MusicBeatSubState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (!submenuOpen)
		{
			...
			{
				OptionsMenuState.overlayBOTTOM.screenCenter();
				OptionsMenuState.overlayBOTTOM.y += 255;
				OptionsMenuState.overlayTOP.screenCenter();
				OptionsMenuState.overlayTOP.y -= 255;
				FlxTween.tween(OptionsMenuState.overlayBOTTOM, {y: 1000}, 1.5, {type: FlxTweenType.BACKWARD, ease: FlxEase.expoOut, loopDelay: 0});
				FlxTween.tween(OptionsMenuState.overlayTOP, {y: -500}, 1.5, {type: FlxTweenType.BACKWARD, ease: FlxEase.expoOut, loopDelay: 0});
				#if android
